...
// Normalize image URL for deduplication — strip tracking/resize params, lowercase hostname
function normalizeImageUrl(url: string): string {
  try {
    const u = new URL(url);
    // Remove common tracking/resize query params
...
// Extract just the filename from a URL for fuzzy dedup
...
// Check if image is a duplicate using normalized URL + filename matching
function isImageDuplicate(newUrl: string, existing: { url: string }[]): boolean {
  ...
  return existing.some((i) => {
    ...
    // Also catch same filename from different CDN subdomains (e.g. img1.example.com vs img2.example.com)
...
export async function GET(req: NextRequest) {
  ...
  for (const img of allResults) {
    ...
    if (!isImageDuplicate(img.url, images)) {
      images.push(img);
    }
  }
